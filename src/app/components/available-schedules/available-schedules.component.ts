...
export class AvailableSchedulesComponent {
  ...
  verificoClases() {
    ...
    this.edit_cronograma.clases.forEach((clase) => {
      ...
      let fecha_clase_format =
        clase.fecha.split('-')[2] + '/' + clase.fecha.split('-')[1] + '/' + clase.fecha.split('-')[0];
      ...
      this.data.forEach((opt) => {
        if (opt.fecha == fecha_clase) {
          ...
          if (this.fecha_no_disponible == true) {
            ...
            this.not_available_classes.push(
              'La clase número ' +
                index_class +
                ' del día ' +
                fecha_clase_format +
                ' a las ' +
                hora_inicio +
                ' hs ya no se encuentra disponible o ya pasó.'
            );
  ...
  saveOptions() {
    this.dataToConfirm = [];
    //Ordeno los dias
    this.classes_send.sort(function (a, b) {
  ...
  armarDirFormateada() {
    ...
    if (this.dataToConfirm[6].address[0].street) {
      ...
      this.dataToConfirm[6].address[5].floor
        ? (direccionFormateada = direccionFormateada + ' Piso: ' + this.dataToConfirm[6].address[5].floor)
        : direccionFormateada + '';
      this.dataToConfirm[6].address[5].floor
        ? (direccionFormateada = direccionFormateada + ' Depto: ' + this.dataToConfirm[6].address[6].department)
        : direccionFormateada + '';

      if (this.dataToConfirm[6].address[1].street_a && this.dataToConfirm[6].address[2].street_b) {
        this.dataToConfirm[6].address[1].diag
          ? (direccionFormateada = direccionFormateada + ' Entre diagonal ' + this.dataToConfirm[6].address[1].street_a)
          : (direccionFormateada = direccionFormateada + ' Entre calle ' + this.dataToConfirm[6].address[1].street_a);

        this.dataToConfirm[6].address[2].diag
          ? (direccionFormateada = direccionFormateada + ' y diagonal ' + this.dataToConfirm[6].address[2].street_b)
          : (direccionFormateada = direccionFormateada + ' y calle ' + this.dataToConfirm[6].address[2].street_b);
      } else {
        if (this.dataToConfirm[6].address[1].street_a) {
          this.dataToConfirm[6].address[1].diag
            ? (direccionFormateada = direccionFormateada + ' Esq diagonal ' + this.dataToConfirm[6].address[1].street_a)
            : (direccionFormateada = direccionFormateada + ' Esq calle ' + this.dataToConfirm[6].address[1].street_a);
        }
        if (this.dataToConfirm[6].address[2].street_b) {
          this.dataToConfirm[6].address[1].diag
            ? (direccionFormateada = direccionFormateada + ' Esq diagonal ' + this.dataToConfirm[6].address[2].street_b)
            : (direccionFormateada = direccionFormateada + ' Esq calle ' + this.dataToConfirm[6].address[2].street_b);
        }
      }
    }
  ...
  armarDirAltFormateada() {
    ...
    if (this.dataToConfirm[8].address_alternative[0].street) {
      ...
      this.dataToConfirm[8].address_alternative[5].floor
        ? (direccionFormateada = direccionFormateada + ' Piso: ' + this.dataToConfirm[8].address_alternative[5].floor)
        : direccionFormateada + '';
      this.dataToConfirm[8].address_alternative[5].floor
        ? (direccionFormateada =
            direccionFormateada + ' Depto: ' + this.dataToConfirm[8].address_alternative[6].department)
        : direccionFormateada + '';

      if (
        this.dataToConfirm[8].address_alternative[1].street_a &&
        this.dataToConfirm[8].address_alternative[2].street_b
      ) {
        this.dataToConfirm[8].address_alternative[1].diag
          ? (direccionFormateada =
              direccionFormateada + ' Entre diagonal ' + this.dataToConfirm[8].address_alternative[1].street_a)
          : (direccionFormateada =
              direccionFormateada + ' Entre calle ' + this.dataToConfirm[8].address_alternative[1].street_a);

        this.dataToConfirm[8].address_alternative[2].diag
          ? (direccionFormateada =
              direccionFormateada + ' y diagonal ' + this.dataToConfirm[8].address_alternative[2].street_b)
          : (direccionFormateada =
              direccionFormateada + ' y calle ' + this.dataToConfirm[8].address_alternative[2].street_b);
      } else {
        if (this.dataToConfirm[8].address_alternative[1].street_a) {
          this.dataToConfirm[8].address_alternative[1].diag
            ? (direccionFormateada =
                direccionFormateada + ' Esq diagonal ' + this.dataToConfirm[8].address_alternative[1].street_a)
            : (direccionFormateada =
                direccionFormateada + ' Esq calle ' + this.dataToConfirm[8].address_alternative[1].street_a);
        }
        if (this.dataToConfirm[8].address_alternative[2].street_b) {
          this.dataToConfirm[8].address_alternative[1].diag
            ? (direccionFormateada =
                direccionFormateada + ' Esq diagonal ' + this.dataToConfirm[8].address_alternative[2].street_b)
            : (direccionFormateada =
                direccionFormateada + ' Esq calle ' + this.dataToConfirm[8].address_alternative[2].street_b);
        }
      }
    }
    return direccionFormateada;
